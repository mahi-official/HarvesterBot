...
def detect_Aruco(img,dictionary):  #returns the detected aruco list dictionary with id: corners
    #added dicitionary parameter for generalization
    aruco_list = {}
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    #added below line for generalizing the code
    path = getattr(aruco, dictionary)
    aruco_dict = aruco.getPredefinedDictionary(path)#aruco.Dictionary_get(aruco.DICT_4X4_50)   #creating aruco_dict with 5x5 bits with max 250 ids..so ids ranges from 0-249
    print(aruco_dict)
